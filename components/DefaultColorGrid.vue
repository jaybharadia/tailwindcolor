<template>
  <div>
    <div class="text-center mb-6 hidden md:block">
      <UBadge
        color="gray"
        icon="ic:outline-keyboard"
        label="Press spacebar to generate new colors"
      />
    </div>
    <div
    ...
    >
      <div v-for="(color, index) in colors" :key="index" class="group relative">
        <ColorShades :base-color="color" @copy="copyHex" />
        <div
    ...
    <button
      class="sm:hidden fixed bottom-6 right-6 bg-primary-500 hover:bg-primary-600 text-white rounded-full w-14 h-14 flex items-center justify-center shadow-lg transform transition-transform hover:scale-105 active:scale-95 pulse-animation"
